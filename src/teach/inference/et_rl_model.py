...
class ETRLModel(TeachModel):
    ...
    def __init__(self, process_index: int, num_processes: int, model_args: List[str]):
        ...
        # parser.add_argument("--model_dir", type=str, default="/home/ubuntu/simbot/teach/exp/teach_et_trial", help="Model folder name under $ET_LOGS")
        # parser.add_argument("--checkpoint", type=str, default="latest.pth", help="latest.pth or model_**.pth")
        # parser.add_argument("--object_predictor", type=str, default="/home/ubuntu/simbot/teach/exp/pretrained/maskrcnn_model.pth", help="Path to MaskRCNN model checkpoint")
        # parser.add_argument("--visual_checkpoint", type=str, default="/home/ubuntu/simbot/teach/exp/pretrained/fasterrcnn_model.pth", help="Path to FasterRCNN model checkpoint")
        # parser.add_argument(
        #     "--skip_edh_history",
        #     action="store_true",
        #     default=False,
        ...
        self.create_model(args, process_index)
    ...
    def create_model(self, args, process_index):
        os.makedirs(self.args.dout, exist_ok=True)
        model_path = os.path.join(self.args.model_dir, self.args.checkpoint)
        logger.info("Loading model from %s" % model_path)
        ...
        if model_path is not None:
            ...
            logger.info(f"Loading model agent using device: {device}")
            #self.model, self.extractor = eval_util.load_agent(model_path, dataset_info, self.args, for_inference=True)
            embs_ann = args.embs_ann
            vocab_out = args.vocab_out
            model, self.extractor, self.optimizer, self.prev_train_info = eval_util.create_agent(self.args, dataset_info, embs_ann, vocab_out)

        self.vocab = {"word": train_vocab["word"], "action_low": model.vocab_out}
        self.preprocessor = Preprocessor(vocab=self.vocab)
        self.model = model.model   # LearnedModel's model

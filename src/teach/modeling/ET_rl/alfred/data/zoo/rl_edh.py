...
class RLEdhDataset(BaseDataset):
    ...
    def load_features(self, task_json):
        ...
        feat = dict()
        # language inputs
        feat["lang"] = RLEdhDataset.load_lang(task_json)

        # action outputs
        if not self.test_mode:
            # low-level action
            feat["action"] = RLEdhDataset.load_action(task_json, self.vocab_out)
